...
angular
  ...
  .controller("PatientsListController", [
    ...
    function (
      ...
      $scope.preferExtraIdInSearchResults = appService
        .getAppDescriptor()
        .getConfigValue("preferExtraIdInSearchResults");
      var personAttributeForPatientSearch = appService
        .getAppDescriptor()
        .getConfigValue("personAttributeForPatientSearch");
      $scope.activeHeaders = [];
      const DEFAULT_FETCH_DELAY = 2000;
      var patientSearchConfig = appService
        .getAppDescriptor()
        .getConfigValue("patientSearch");
      var patientListSpinner;
      var initialize = function () {
        var searchTypes = appService
          .getAppDescriptor()
          .getExtensions("org.bahmni.patient.search", "config")
          .map(mapExtensionToSearchType);
        $scope.search = new Bahmni.Common.PatientSearch.Search(
          _.without(searchTypes, undefined)
        );
        ...
        if (patientSearchConfig && patientSearchConfig.serializeSearch) {
          ...
          _.each($scope.search.searchTypes, function (searchType) {
            _.isEmpty(searchType) ||
              ($scope.search.searchType != searchType &&
                getPatientCount(searchType, null));
          });
        }
        ...
        configurationService
          .getConfigurations(["identifierTypesConfig"])
        ...
        return spinner
          .forPromise(
            patientService.search(
              ...
              personAttributeForPatientSearch
            )
          )
          .then(function (response) {
      ...
      var getPatientCount = function (searchType, patientListSpinner) {
        if (searchType.handler) {
          var params = {
            ...
            v: "full",
            location_uuid: $bahmniCookieStore.get(
              Bahmni.Common.Constants.locationCookieName
            ).uuid,
      ...
      $scope.getHeadings = function () {
        if (
          $scope.search.activePatients &&
          $scope.search.activePatients.length > 0
        ) {
      ...
      var setActiveHeadings = function (headings) {
        headings.map(function (heading) {
          ...
          if (
            !$scope.activeHeaders.find(function (activeHeader) {
              return (
                activeHeader.name == newHeading.name &&
                activeHeader.sortInfo == newHeading.sortInfo
              );
            })
          ) {
      ...
      $scope.sortVisiblePatientsBy = function (sortColumn) {
        ...
        var nonEmptyObjects = _.difference(
          $scope.search.searchResults,
          emptyObjects
        );
        var sortedNonEmptyObjects = _.sortBy(
          nonEmptyObjects,
          function (visiblePatient) {
        ...
        $scope.search.visiblePatients =
          sortedNonEmptyObjects.concat(emptyObjects);
        $scope.sortColumn = sortColumn;
        $scope.reverseSort = !$scope.reverseSort;
      };

      $scope.isHeadingOfLinkColumn = function (heading) {
        var identifierHeading = _.includes(
          Bahmni.Common.PatientSearch.Constants.identifierHeading,
          heading
        );
        if (identifierHeading) {
          return identifierHeading;
        } else if ($scope.search.searchType && $scope.search.searchType.links) {
          return _.find($scope.search.searchType.links, {
            linkColumn: heading,
          });
        } else if (
          $scope.search.searchType &&
          $scope.search.searchType.linkColumn
        ) {
      ...
      $scope.isHeadingOfName = function (heading) {
        return _.includes(
          Bahmni.Common.PatientSearch.Constants.nameHeading,
          heading
        );
      };
      $scope.getPrintableHeadings = function () {
        ...
        var printableHeadings = $scope.activeHeaders.filter(function (heading) {
          return (
            _.indexOf(
              Bahmni.Common.PatientSearch.Constants.printIgnoreHeadingsList,
              heading.name
            ) === -1
          );
        });
      ...
      $scope.printPage = function () {
        if ($scope.search.searchType.printHtmlLocation != null) {
          printer.printFromScope(
            $scope.search.searchType.printHtmlLocation,
            $scope
          );
        }
      };
      ...
      var mapExtensionToSearchType = function (appExtn) {
        return {
          ...
          view:
            appExtn.extensionParams.view ||
            Bahmni.Common.PatientSearch.Constants.searchExtensionTileViewType,
      ...
      var debounceGetPatientCount = _.debounce(
        function (currentSearchType, patientListSpinner) {
          getPatientCount(currentSearchType, patientListSpinner);
        },
        (patientSearchConfig && patientSearchConfig.fetchDelay) ||
          DEFAULT_FETCH_DELAY,
        {}
      );
      ...
      $scope.forwardPatient = function (patient, heading) {
        ...
        if ($scope.search.searchType.links) {
          link =
            _.find($scope.search.searchType.links, { linkColumn: heading }) ||
            _.find($scope.search.searchType.links, {
              linkColumn: heading.name,
            }) ||
            link;
        }
        ...
        if (link.url && link.url !== null) {
          if (typeof link.url === "object") {
            const rowName = patient[heading.name]
              ?.replace(/\s/g, "")
              ?.toLowerCase();
            const redirectUrl =
              rowName && link.url[rowName]
                ? link.url[rowName]
                : link.url.default;
            var newWindow = $window.open(
              appService
                .getAppDescriptor()
                .formatUrl(redirectUrl, options, true),
              link.newTab
                ? "_blank"
                : link.targetedTab
                ? link.targetedTab
            ...
            var newWindow = $window.open(
              appService.getAppDescriptor().formatUrl(link.url, options, true),
              link.newTab
                ? "_blank"
                : link.targetedTab
                ? link.targetedTab
