...
angular.module('bahmni.common.patientSearch').controller('PatientsListController', [
  '$scope',
  '$window',
  'patientService',
  '$rootScope',
  ...
  function (
      ...
      if (patientSearchConfig && patientSearchConfig.serializeSearch) {
        ...
        _.each($scope.search.searchTypes, function (searchType) {
          _.isEmpty(searchType) ||
            ($scope.search.searchType != searchType && getPatientCount(searchType, null));
        });
      }
    ...
    $scope.searchPatients = function () {
      return spinner
        .forPromise(
          patientService.search($scope.search.searchParameter, personAttributeForPatientSearch),
        )
      ...
      if ($scope.search.activePatients && $scope.search.activePatients.length > 0) {
        var headings = _.chain($scope.search.activePatients[0])
          ...
          .filter(function (heading) {
            return (
              _.indexOf(
                Bahmni.Common.PatientSearch.Constants.tabularViewIgnoreHeadingsList,
                heading,
              ) === -1
            );
          })
      ...
      headings.map(function (heading) {
        ...
        if (
          !$scope.activeHeaders.find(function (activeHeader) {
            return (
              activeHeader.name == newHeading.name && activeHeader.sortInfo == newHeading.sortInfo
            );
          })
        ) {
    ...
    $scope.getPrintableHeadings = function () {
      ...
      var printableHeadings = $scope.activeHeaders.filter(function (heading) {
        return (
          _.indexOf(Bahmni.Common.PatientSearch.Constants.printIgnoreHeadingsList, heading.name) ===
          -1
        );
      });
    ...
    $scope.forwardPatient = function (patient, heading) {
      ...
      if ($scope.search.searchType.links) {
        link =
          ...
          _.find($scope.search.searchType.links, { linkColumn: heading.name }) ||
          link;
      }
      ...
      if (link.url && link.url !== null) {
        if (typeof link.url === 'object') {
          ...
          const redirectUrl = rowName && link.url[rowName] ? link.url[rowName] : link.url.default;
