...
angular.module('bahmni.reports')
    .controller('MonthlyReportsController', ['$scope', 'appService', 'reportService', 'FileUploader', 'messagingService', 'spinner', '$rootScope', 'auditLogService', function ($scope, appService, reportService, FileUploader, messagingService, spinner, $rootScope, auditLogService) {
        var appName = "monthlyReports";
        ...
        $rootScope.default = _.isUndefined($rootScope.default) ? {monthlyReportsRequiringDateRange: {}, monthlyReportsNotRequiringDateRange: {}} : $rootScope.default;
        ...
        var isDateRangeRequiredFor = function (report) {
            return _.find($rootScope.monthlyReportsRequiringDateRange, {name: report.name});
        };
        ...
        var initialization = function () {
            var reportList = appService.getAppDescriptor().getConfigForPage("monthlyReports");
            $rootScope.monthlyReportsRequiringDateRange = _.isUndefined($rootScope.monthlyReportsRequiringDateRange) ? _.values(reportList).filter(function (report) {
                return !(report.config && report.config.dateRangeRequired === false);
            }) : $rootScope.monthlyReportsRequiringDateRange;
            $rootScope.monthlyReportsNotRequiringDateRange = _.isUndefined($rootScope.monthlyReportsNotRequiringDateRange) ? _.values(reportList).filter(function (report) {
                return (report.config && report.config.dateRangeRequired === false);
            }) : $rootScope.monthlyReportsNotRequiringDateRange;
