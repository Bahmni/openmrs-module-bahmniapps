...
angular.module('bahmni.clinical').factory('ordersTabInitialization',
    ['conceptSetService', 'spinner', function(conceptSetService, spinner){
        return function(){
            ...
            var allOrderables = spinner.forPromise(conceptSetService.getConceptSetMembers({name:"All Orderables",v:"custom:(uuid,name,names,conceptClass,setMembers:(uuid,name,names,conceptClass,setMembers:(uuid,name,names,conceptClass,setMembers:(uuid,name,names,conceptClass))))"})).then(function (response) {
                var allOrderables = {};
