...
describe("AddTreatmentController", function () {
    ...
    describe("Detect Overlapping orders amongst new orders on Save", function () {
        describe("should allow potentially overlapping order whose dates can be set and be resolved", function () {
            ...
            it("new drug orders for dates 2-4 and 5-6 and 4-5 in this order", function () {
                var dec2Dec4order = Bahmni.Tests.drugOrderViewModelMother.buildWith({},
                    {
                        drug: {name: "abc", uuid: "123"},
                        effectiveStartDate: DateUtil.parse("2014-12-02"),
                        effectiveStopDate: DateUtil.parse("2014-12-04"),
                    ...
                    encounterDate);
                var dec5Dec6order = Bahmni.Tests.drugOrderViewModelMother.buildWith({},
                    {
                        drug: {name: "abc", uuid: "123"},
                    ...
                    encounterDate);
                var dec4Dec5order = Bahmni.Tests.drugOrderViewModelMother.buildWith({},
                    {
                        drug: {name: "abc", uuid: "123"},
                        effectiveStartDate: DateUtil.parse("2014-12-04"),
                ...
                scope.treatment = dec2Dec4order;
                expect(scope.treatments.length).toEqual(0);
                scope.add();

                expect(scope.treatment).not.toEqual(dec2Dec4order);
                expect(scope.treatments.length).toEqual(1);
                var dec2Dec4orderAfterSave = scope.treatments[0];
                expect(DateUtil.isSameDateTime(dec2Dec4orderAfterSave.effectiveStartDate, DateUtil.parse("2014-12-02"))).toBeTruthy();
                expect(DateUtil.isSameDateTime(dec2Dec4orderAfterSave.effectiveStopDate, DateUtil.parse("2014-12-04"))).toBeTruthy();
                expect(dec2Dec4orderAfterSave.scheduledDate).toEqual(dec2Dec4orderAfterSave.effectiveStartDate);
                expect(dec2Dec4orderAfterSave.autoExpireDate).toBeUndefined();

                scope.treatment = dec5Dec6order;
                scope.add();

                expect(scope.treatment).not.toEqual(dec5Dec6order);
                expect(scope.treatments.length).toEqual(2);
                var dec5Dec6orderAfterSave = scope.treatments.filter(function (treatment) {
                    return DateUtil.isSameDate(treatment.effectiveStartDate, "2014-12-05");
                })[0];

                expect(DateUtil.isSameDateTime(dec5Dec6orderAfterSave.effectiveStartDate, DateUtil.parse("2014-12-05"))).toBeTruthy();
                expect(DateUtil.isSameDateTime(dec5Dec6orderAfterSave.effectiveStopDate, DateUtil.parse("2014-12-06"))).toBeTruthy();
                expect(DateUtil.isSameDateTime(dec5Dec6orderAfterSave.scheduledDate, DateUtil.parse("2014-12-05"))).toBeTruthy();
                expect(dec5Dec6orderAfterSave.autoExpireDate).toBeUndefined();

                scope.treatment = dec4Dec5order;
                scope.add();

                expect(scope.treatment).not.toEqual(dec4Dec5order);
                expect(scope.treatments.length).toEqual(3);
                var dec4Dec5orderAfterSave = scope.treatments.filter(function (treatment) {
                    return DateUtil.isSameDate(treatment.effectiveStartDate, "2014-12-04");
                })[0];

                expect(DateUtil.isSameDateTime(dec4Dec5orderAfterSave.effectiveStartDate, DateUtil.parse("2014-12-04"))).toBeTruthy();
                expect(DateUtil.isSameDateTime(dec4Dec5orderAfterSave.effectiveStopDate, DateUtil.subtractSeconds("2014-12-05", 1))).toBeTruthy();
                expect(DateUtil.isSameDateTime(dec4Dec5orderAfterSave.scheduledDate, DateUtil.parse("2014-12-04"))).toBeTruthy();
                expect(dec4Dec5orderAfterSave.autoExpireDate).toBeUndefined();
            });
            ...
            it("new scheduled drug orders for dates 2-4 and 5-6 and 4-5 in this order with past encounter date", function () {
                ...
                expect(DateUtil.isSameDateTime(dec2Dec4orderAfterSave.effectiveStartDate, DateUtil.parse("2014-12-02"))).toBeTruthy();
                expect(DateUtil.isSameDateTime(dec2Dec4orderAfterSave.effectiveStopDate, DateUtil.parse("2014-12-04"))).toBeTruthy();
                expect(DateUtil.isSameDateTime(dec2Dec4orderAfterSave.scheduledDate, DateUtil.parse("2014-12-02"))).toBeTruthy();
                expect(dec2Dec4orderAfterSave.autoExpireDate).toBeUndefined();
        ...
        describe("should allow potentially overlapping order whose dates can be set and be resolved", function () {
            ...
            it("existing drug orders for dates 2-3 and 3-4 and revised drug order for 2-3", function () {
                var dec2Dec3order = Bahmni.Tests.drugOrderViewModelMother.buildWith({}, {
                    drug: {
                        name: "abc",
                        uuid: "123"
                    },
                ...
                var dec3Dec4order = Bahmni.Tests.drugOrderViewModelMother.buildWith({}, {
                    drug: {
                        name: "abc",
                        uuid: "123"
                    },
                ...
                scope.consultation.activeAndScheduledDrugOrders = [dec2Dec3order, dec3Dec4order];

                expect(scope.treatments.length).toEqual(0);

                scope.revise(dec2Dec3order, scope.consultation.activeAndScheduledDrugOrders);
                expect(dec2Dec3order.isBeingEdited).toBeTruthy();
                dec2Dec3order.effectiveStartDate = DateUtil.parse("2014-12-02 11:00:00");
                scope.add();
            ...
            it("existing drug orders 4-5 and new drug order 2-4(starting today)", function () {
                ...
                scope.consultation.activeAndScheduledDrugOrders = [dec4Dec5order];
                ...
                scope.add();
                expect(scope.treatments.length).toEqual(1);
                expect(DateUtil.isSameDateTime(dec2Dec4order.autoExpireDate, DateUtil.subtractSeconds("2014-12-04 00:00:00", 1))).toBeTruthy();
            });
        });
        ...
        describe("should not allow overlapping order", function () {
            ...
            it("new orders for dates 2-4 and 3-6", function () {
                ...
                var dec3Dec6order = Bahmni.Tests.drugOrderViewModelMother.buildWith({}, {
                    drug: {
                        name: "abc",
                        uuid: "123"
                    },
    ...
    describe("Detect Overlapping orders with existing Orders on Save", function () {
        describe("should allow potentially overlapping order whose dates can be set and be resolved", function () {
            ...
            it("existing drug orders for dates 2-4 and 5-6 and new drug order for 4-5", function () {
                ...
                scope.treatments = [dec2Dec4order, dec5Dec6order];
        ...
        describe("should not allow overlapping order", function () {
            it("should not allow conflicting orders for dates 2-4 and new drug order for 3-6", function () {
                ...
                scope.treatments = [dec2Dec4order];
    ...
    describe("After selection from ng-dialog", function () {
        ...
        it("should edit the drug order if conflicting order is unsaved when revise is selected", function () {
            ...
            var newDec2Dec4order = Bahmni.Tests.drugOrderViewModelMother.buildWith({}, {
                drug: {
                    name: "abc",
                    uuid: "123"
                },
            ...
            scope.treatment = newDec2Dec4order;
            scope.add();
            expect(scope.treatments.length).toEqual(1);
            expect(ngDialog.open).toHaveBeenCalled();

            scope.revise(dec2Dec4order, 0);
            expect(ngDialog.close).toHaveBeenCalled();
            expect(dec2Dec4order.isBeingEdited).toBeTruthy();
        ...
        it("should edit the drug order if conflicting order is a saved order when revise is selected", function () {
            ...
            scope.consultation.activeAndScheduledDrugOrders = [dec2Dec4order];
            expect(scope.treatments.length).toEqual(0);
            ...
            scope.add();
            expect(scope.treatments.length).toEqual(0);
            expect(scope.alreadyActiveSimilarOrder).toBe(dec2Dec4order);
            expect(ngDialog.open).toHaveBeenCalled();
